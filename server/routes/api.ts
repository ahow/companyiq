...
router.get("/debug/jobs", async (req: Request, res: Response) => {
  try {
    ...
    `);
    const latestBatch = await db.execute(sql`SELECT id FROM batch_runs ORDER BY id DESC LIMIT 1`);
    const latestBatchId = latestBatch.rows[0]?.id || 0;
    const batch5Jobs = await db.execute(sql`SELECT id, company_id, company_name, status, last_error, worker_id, claimed_at, completed_at, batch_id FROM analysis_jobs WHERE batch_id = ${latestBatchId} ORDER BY status DESC, id LIMIT 50`);
